...
На сайте подключён `js/metrics.js` после основного скрипта на каждой из трёх страниц. Добавлены метрики: `nav_click` для пунктов меню, `utm_visit` для параметров кампании, `read_30s` для чтения страницы 30 секунд и `form_error` для невалидной формы. Для проверки UTM использована ссылка с `utm_source=telegram`, `utm_medium=social`, `utm_campaign=ga4_lab`; параметры передаются в GA4 вместе с `landing_page`.

Эти метрики показывают маршрут посетителя, канал входа, глубину чтения и места, где посетитель ошибается в форме. События также доступны в Realtime и DebugView после включения режима отладки.
