...
Сайт для проверки: https://lastuser55556.github.io/ga4-analytics-lab/ . В Network проверены запросы Google Analytics: при открытии страницы отправляется `page_view`, при первом визите — `first_visit` и `session_start`, при прокрутке — `scroll`, при переходе в меню — `nav_click`. В Payload `page_view` передаётся `dl` с адресом страницы, а в пользовательских событиях — название ссылки и страница.

Путь пользователя: ссылка с UTM → Home → About → Contacts → заполнение формы → отправка заявки в Google Таблицы. Критическая точка потери данных — переход без UTM: источник заявки нельзя связать с каналом привлечения. Для исправления использованы `utm_source`, `utm_medium` и `utm_campaign`, которые сохраняются вместе с заявкой.
